...
export class UcOrganizeEvent {
  ...
  public eventPanelShowHide(): void {
    ...
    this.addSubscriberEvent.bind(this)();
  }

  /**
   * Ajoute un participant à l'événement
   */
  private addSubscriberEvent(): void{
    if(this.crewLetter.ref != '' && this.crewId.ref != 0){
      const index = this.crews.ref.findIndex(x => x.getId() == this.crewId.ref);
      var subscribers = this.event.ref.getSubscribers().slice(0);
      subscribers.push(new Subscriber(this.subscriberService.getNewID(), this.crews.ref[index].getName(), this.crews.ref[index].getCountry(), this.crews.ref[index].getIsaf(), this.crews.ref[index].getRanking(), this.crewLetter.ref));
      this.event.ref.setSubscribers(subscribers.slice(0));
      this.crewLetter.ref = '';
      this.crewId.ref = 0;
    }
    if(!this.eventPanelIhm.ref)
      setTimeout(this.addSubscriberEvent.bind(this), TIME_TO_WAIT_BUTTON);
  }
