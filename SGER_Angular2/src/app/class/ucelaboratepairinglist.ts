...
export class UcElaboratePairingList {
   ...
   * @param updateMenu Référence de la fonction
   * @param event Evénemen sélectionner
   * @param phase Phase sélectionner
   */
  private updateMenu(updateMenu: any, event: {ref: Event}, phase: {ref: Phase}): void{
   ...
   * @param autoCreateMatch Référence de la fonction
   * @param matchService Persistance des matchs
   * @param racerService Persistance des concurrent
   * @param event Evénement sélectionné
   * @param phase Phase sélectionnée
   */
    ...
    for (var i = 0; i < fligths.length; i++) {
      if(fligths[i].getHiddenContent()){
        if(fligths[i].getBlueSubscriberId() > 0 && fligths[i].getBlueBoatId() > 0 && fligths[i].getYellowSubscriberId() > 0 && fligths[i].getYellowBoatId() > 0){
          ...
          matchService.setFlightID(match, fligths[i]);
   ...
   * @param autoDeleteMatch Référence de la fonction
   * @param matchService Persistance des matchs
   * @param phase Phase sélectionnée
   */
