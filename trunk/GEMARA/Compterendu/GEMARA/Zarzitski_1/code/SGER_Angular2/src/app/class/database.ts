...
export class DataBase {
   ...
   * @param setResult La fonction qui doit être lancée à la fin du chargement
   * @param dataID Le nom du service
   */
  public readDB(request: RequestSQL, entitys: {ref: any}, applyResult, divID: string): void {
    SQLite.read(request, divID);
    this.waitResult.bind(this)(divID, entitys, applyResult);
  }
   ...
   * @param setResult La fonction qui doit être lancée à la fin du chargement
   */
  private waitResult(divID: string, entitys: any, setResult){
    if(document.getElementById(divID).innerHTML == '')
      setTimeout(this.waitResult.bind(this, divID, entitys, setResult), TIME_TO_WAIT);
    else
      setResult(entitys, divID);
  }
